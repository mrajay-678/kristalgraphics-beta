...
export default function DynamicBentoGrid({ items }: Props) {
  const getSpan = (width: number, height: number, index: number) => {
  ...
  return (
    <div className="grid grid-cols-12 auto-rows-[120px] gap-4">
      {items.map((item, index) => (
        <BentoCard key={item.id} item={item} index={index} getSpan={getSpan} />
      ))}
    </div>
  );
}
...
function BentoCard({
  ...
  item: BentoItem;
  index: number;
  getSpan: (width: number, height: number, index: number) => string;
}) {
  ...
  return (
    <div
      ...
      `}
    >
      <div className="relative min-h-max h-3/4 w-full">
